'''
Собирает заголовки и URL'ы объявлений из вьюдира по городам. За счет прохождения по отдельным городам дает гораздо больше заголовков, чем при простом обходе по всей России.
Вход: URL вьюдира
Выход: файл со списком заголовков и URL'ов
...
#загружаем урлы городов из внешнего файла
...
def collect_titles(city_url):
    '''
    Собирает заголовки и URL'ы объявлений из вьюдира
    Вход: URL вьюдира
    Выход: список из заголовков и URL'ов
    '''
    i = 1
    titles_list_raw = []
    titles_list = []
    #получаем число вьюбулов для расчета количества страниц
    ...
    while i <= page_total:
        ...
        for line in source_page_for_titles:
            titles_list_raw += re.findall('(.\d+.html.*?>.*?)</a>', line.decode('cp1251'))
...
with open(results, 'a') as f:
    for city in city_list:
        print()
        city = city[22:-1] #удалаяем фразу 'https://farpost.ru' с начала строки и '\n' с конца строки
